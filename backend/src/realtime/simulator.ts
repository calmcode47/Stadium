...
export const startSimulator = (): SimulatorHandle => {
  ...
  const interval = setInterval(() => {
    ...
    const liveMatches = db.select().from(matches).all().filter(match => match.status === 'live')
    ...
    if (updated) broadcast('match:updated', mapMatch(updated))
    ...
    if (Math.random() < 0.15) {
      ...
      if (alert) broadcast('alert:created', mapAlert(alert))
    }

    broadcast('assistant:recommendations-changed', { reason: 'simulator-tick' })
  }, 8000)
