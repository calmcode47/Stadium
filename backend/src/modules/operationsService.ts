...
export const getOperationsState = (tournamentId = 'T-CHAMPIONS-2026'): OperationsState => {
  ...
  const roundRows = db.select().from(rounds).where(eq(rounds.tournamentId, tournamentId)).all()
  const tournamentMatchRows = matchRows.filter(match => match.tournamentId === tournamentId && match.roundId !== null)
